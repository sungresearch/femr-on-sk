...
def load_features(
    ...
    When train_n is specified (e.g., for few_shot experiments), train_perc
    is ignored and balanced sampling is conducted to obtain the labels 
    ...
    if is_eval:
        # always use the same 15% of patients for evaluation
        sel_pids_idx = np.where(hashed_pids >= 85)[0]
    ...
    if train_n is not None:
        #if train_n is specified, we are sampling train_n samples
